...
class OxfordDictionaryStorage @Inject constructor(
    ...
    fun getShortWordInfo(word: String): Single<ShortWordInfo> = Single.just(word)
            .flatMap {
                val wordDetails = wordsCache.get(word)
                if (wordDetails == null) {
            ...
            .map {
                ShortWordInfo(word, it.meanings.map {
                    WordMeaning(it.definitionId, it.definitions, it.partOfSpeech ?: "", it.examples)
                }, it.pronunciation)
            }

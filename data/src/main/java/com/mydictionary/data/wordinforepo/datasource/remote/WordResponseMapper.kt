...
object WordInfoMapper {
    fun map(wordDetailsResponse: WordDetailsResponse?): ShortWordInfo {
        ...
        allEntries.forEach { entry ->
            ...
            entry.senses?.filter { it.definitions?.isNotEmpty() == true }?.forEach {
                ...
                val meaning = WordMeaning(it.id, it.definitions, entry.lexicalCategory
                        ?: "", examples)
    ...
    fun map(wordDetailsResponse: WordDetailsResponse, relatedWordsResponse: RelatedWordsResponse?)
        ...
        return DetailWordInfo(shortWordInfo.word, shortWordInfo.meanings,
    ...
    fun zipTwoResponseResults() =
            ...
            { wordDetailsResponse, relatedWordsResponse ->
                Pair<WordDetailsResponse, RelatedWordsResponse?>(wordDetailsResponse, relatedWordsResponse)
            }
}
